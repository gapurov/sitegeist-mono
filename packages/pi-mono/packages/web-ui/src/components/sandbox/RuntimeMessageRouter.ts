...
export class RuntimeMessageRouter {
	...
	unregisterSandbox(sandboxId: string): void {
		...
		if (this.sandboxes.size === 0) {
			...
			const userScriptMessages = typeof chrome !== "undefined" ? chrome.runtime?.onUserScriptMessage : undefined;
			if (this.userScriptMessageListener && userScriptMessages) {
				userScriptMessages.removeListener(this.userScriptMessageListener);
